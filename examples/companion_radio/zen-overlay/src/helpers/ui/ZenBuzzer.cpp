...
void ZenBuzzer::quiet(bool buzzer_state) {
...
bool ZenBuzzer::isQuiet() { return _is_quiet; }

void ZenBuzzer::startup()  { play(startup_song); }
void ZenBuzzer::shutdown() { play(shutdown_song); }
...
uint16_t ZenBuzzer::_noteFreq(char letter, bool sharp, uint8_t octave) {
...
void ZenBuzzer::_parseHeader(const char* melody, uint8_t& def_dur, uint8_t& def_oct,
...
bool ZenBuzzer::_parseNext(const char*& p, uint8_t def_dur, uint8_t def_oct, uint16_t bpm,
...
uint8_t ZenBuzzer::_dutyPct() const {
...
void ZenBuzzer::_nrfStartPwm(uint16_t freq) {
...
ZenBuzzer* ZenBuzzer::_isr_instance = nullptr;

void ZenBuzzer::_armNoteTimer(uint32_t dur_ms) {
    NRF_TIMER1->TASKS_STOP  = 1;
...
void ZenBuzzer::_disarmNoteTimer() {
    NRF_TIMER1->TASKS_STOP = 1;
    NRF_TIMER1->EVENTS_COMPARE[0] = 0;
...
// friend declaration — same trick ZenMomentaryButton's isrTrampolineN() uses.
// The real ISR (TIMER1_IRQHandler, below) is just a one-line dispatch to this.
void ZenBuzzer::_timer1ISR() {
    NRF_TIMER1->EVENTS_COMPARE[0] = 0;
...
void ZenBuzzer::_nrfBegin(const char* melody) {
...
void ZenBuzzer::play(const char* melody) {
    if (_is_quiet) return;
    _nrfBegin(melody);
}

void ZenBuzzer::playForced(const char* melody) {
    _nrfBegin(melody);
}

bool ZenBuzzer::isPlaying() { return !_rtttl_done; }
...
extern "C" void TIMER1_IRQHandler(void) {
    ZenBuzzer::_timer1ISR();
}

void ZenBuzzer::setVolume(uint8_t level) {
...
bool ZenBuzzer::isPlaying() { return rtttl::isPlaying(); }

void ZenBuzzer::stop() { rtttl::stop(); }
