#include "ZenSH1106Display.h"
#include <Adafruit_GrayOLED.h>
#include "Adafruit_SH110X.h"
#include "MiscFixedRenderer.h"

bool ZenSH1106Display::i2c_probe(TwoWire &wire, uint8_t addr)
{
...
void ZenSH1106Display::turnOff()
{
  display.oled_command(SH110X_DISPLAYOFF);
  _isOn = false;
}

void ZenSH1106Display::clear()
{
...
void ZenSH1106Display::startFrame(ColorVal bkg)
{
...
void ZenSH1106Display::setTextSize(int sz)
{
  _text_sz = sz;
  _vw_dirty = true;
...
void ZenSH1106Display::setColor(ColorVal c)
{
  _color = c;
  display.setTextColor(_color);
}

void ZenSH1106Display::setCursor(int x, int y)
{
  display.setCursor(x, y);
}

uint8_t ZenSH1106Display::glyphXAdvance(uint32_t cp) {
  return miscFixedXAdvance(cp, _text_sz);
}

void ZenSH1106Display::print(const char *str)
{
  miscFixedPrint(display, str, _text_sz, _color);
}

void ZenSH1106Display::fillRect(int x, int y, int w, int h)
{
  display.fillRect(x, y, w, h, _color);
}

void ZenSH1106Display::drawRect(int x, int y, int w, int h)
{
  display.drawRect(x, y, w, h, _color);
}

void ZenSH1106Display::drawXbm(int x, int y, const uint8_t *bits, int w, int h)
{
  display.drawBitmap(x, y, bits, w, h, _color);
}

uint16_t ZenSH1106Display::getTextWidth(const char *str)
{
  return miscFixedTextWidth(str, _text_sz);
}

void ZenSH1106Display::setBrightness(uint8_t level)
{
...
void ZenSH1106Display::endFrame()
{
