...
inline int miniIconScale(ZenDisplayDriver& d) {
...
inline void miniIconDraw(ZenDisplayDriver& d, int x, int top_y,
...
inline void miniIconDraw(ZenDisplayDriver& d, int x, int top_y, const MiniIcon& ic) {
...
inline void miniIconDrawTop(ZenDisplayDriver& d, int x, int y, const MiniIcon& ic) {
  const int s = miniIconScale(d);
...
inline void miniIconDrawCentered(ZenDisplayDriver& d, int cx, int cy, const MiniIcon& ic) {
  const int s = miniIconScale(d);
...
inline void drawSlotIcon(ZenDisplayDriver& d, int x, int box_w, int box_h, const MiniIcon& ic) {
  const int s = miniIconScale(d);
...
inline void drawBoxedIcon(ZenDisplayDriver& d, int x, int box_w, int box_h, const MiniIcon& ic) {
...
inline void miniIconDotRow(ZenDisplayDriver& d, int x, int top_y, int count) {
  const int s = miniIconScale(d);
...
// triangle + 1px gap).
inline int scrollIndicatorColWidth(ZenDisplayDriver& d) {
  return 5 * miniIconScale(d) + 1;            // triangle (5*scale) + 1px gap
}
inline int scrollIndicatorReserve(ZenDisplayDriver& d, int total, int visible) {
...
// ZenDisplayDriver::drawScrollArrows that also shows how much of the list is on
...
inline void drawScrollIndicatorPx(ZenDisplayDriver& d, int right_x, int top_y, int track_h,
...
inline void drawScrollIndicatorPx(ZenDisplayDriver& d, int top_y, int track_h,
                                  long total_px, long view_px, long scroll_px) {
...
// ZenDisplayDriver::drawScrollArrows.
//   total   : total number of items
//   visible : items shown at once
//   first   : index of the first visible item (scroll offset)
inline void drawScrollIndicator(ZenDisplayDriver& d, int top_y, int track_h,
...
inline void drawScrollIndicator(ZenDisplayDriver& d, int right_x, int top_y, int track_h,
                                int total, int visible, int first) {
...
inline int drawListAt(ZenDisplayDriver& d, int start_y, int total, int sel,
...
template <class RenderRow>
inline int drawList(ZenDisplayDriver& d, int total, int sel, int& scroll, RenderRow row) {
...
inline void drawLabelValueRow(ZenDisplayDriver& d, int y, const char* label,
...
inline void drawRowSelection(ZenDisplayDriver& d, int y, bool sel, int reserve) {
...
inline void bigIconDraw(ZenDisplayDriver& d, int x, int y, const BigIcon& ic) {
  for (int r = 0; r < ic.h; r++)
    for (int c = 0; c < ic.w; c++)
...
inline int favStarWidth(ZenDisplayDriver& display) {
  return ICON_PG_STAR.w * miniIconScale(display) + 2;
}

inline void drawFavStar(ZenDisplayDriver& display, int x, int y) {
