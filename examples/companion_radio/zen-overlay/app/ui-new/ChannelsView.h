...
class ChannelsView {
  ...
  void openKb(const char* initial, int max) {
    kb().beginProfile(zen::EditorProfile::LITERAL, initial, max);
    _kb_active = true;
  }
  ...
  void commitPublic() {
    ...
    if (result == MyMesh::CHANNEL_SAVED) _task->showAlert("Channel added", 1000);
      ...
      _task->operationWarning(zen::Operation::CHANNEL, zen::OperationReason::DUPLICATE);
    else {
      _task->operationError(zen::Operation::CHANNEL,
          zen::OperationOutcome::TRANSPORT_FAILURE, zen::OperationReason::SAVE_FAILED);
    }
    _mode = OFF;
  }
  ...
  void commit() {
    if (_mode == ADD_HASHTAG) {
      if (_topic[0] == '\0') {
        _task->operationWarning(zen::Operation::CHANNEL, zen::OperationReason::REQUIRED);
        return;
      }
    ...
    if (!deriveSecret(secret)) {
      _task->operationWarning(zen::Operation::CHANNEL,
          _hex_mode ? zen::OperationReason::INVALID_SECRET
