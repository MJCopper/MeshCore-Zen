...
#include "../zen/RadioUiPolicy.h"
#include "../zen/BuiltinMelodies.h"
#include "../zen/TimezonePolicy.h"

class SettingsScreen : public ZenUIScreen {
  UITask* _task;
  ...
  static const int NUM_SECTIONS = 10 + ZEN_FEATURE_CHILD_MODE;
  ...
  RadioPresetPicker::Target radioTarget(ZenPrefs* p) const {
  ...
  int valCol(ZenDisplayDriver& display) const { return display.valCol() - _reserve; }

  static uint32_t prefsFingerprint(const ZenPrefs& prefs) {
    const uint8_t* bytes = reinterpret_cast<const uint8_t*>(&prefs);
    uint32_t hash = 2166136261UL;
    for (size_t i = 0; i < sizeof(ZenPrefs); i++) {
      hash ^= bytes[i];
      hash *= 16777619UL;
    }
    return hash;
  ...
  void renderBar(ZenDisplayDriver& display, int x, int y, int value, int max_val) {
    const int gap     = 2;
  ...
  uint16_t homePageBit(int item) const {
    ...
    if (bit < 0 || bit == ZenPrefs::HPB_SETTINGS || bit == ZenPrefs::HPB_QUICK_MSG) return 0;
    return (uint16_t)(1 << bit);
  }

  const char* homePageLabel(int item) const {
    int bit = homePageBitIndex(item);
    return ZenPrefs::homePageLabel((uint8_t)(bit >= 0 ? bit : ZenPrefs::HPB_COUNT));
  }

  bool homePageVisible(int item, const ZenPrefs* p) const {
  ...
  // Bit-index values are defined once in ZenPrefs::HomePageBit.
  int homePageBitIndex(int item) const {
    if (item == HOME_SENSORS)   return ZenPrefs::HPB_SENSORS;
    if (item == HOME_FAVOURITES) return ZenPrefs::HPB_FAVOURITES;
    if (item == HOME_RADIO)     return ZenPrefs::HPB_RADIO;
    if (item == HOME_BT)        return ZenPrefs::HPB_BLUETOOTH;
    if (item == HOME_ADVERT)    return ZenPrefs::HPB_ADVERT;
#if ENV_INCLUDE_GPS == 1
    if (item == HOME_GPS)       return ZenPrefs::HPB_GPS;
#endif
    if (item == HOME_TOOLS)     return ZenPrefs::HPB_TOOLS;
    if (item == HOME_SETTINGS)  return ZenPrefs::HPB_SETTINGS;
    if (item == HOME_QUICK_MSG) return ZenPrefs::HPB_QUICK_MSG;
    return -1;
  }

  // Returns 1-based position of item in page_order, or 0 if no custom order / not found.
  int homePagePosition(int item, const ZenPrefs* p) const {
    if (!p || p->page_order_set != ZenPrefs::PAGE_ORDER_MAGIC) return 0;
    int bit = homePageBitIndex(item);
    if (bit < 0) return 0;
  ...
  void ensurePageOrderInit(ZenPrefs* p) const { homepage::ensureOrder(p); }

  // Swaps item's page_order slot with its neighbour in the given direction (-1=earlier, +1=later).
  void movePageInOrder(int item, int delta, ZenPrefs* p) {
    int bit = homePageBitIndex(item);
    if (bit < 0) return;
  ...
  bool isMsgSlot(int item) const {
    return item >= MSG_SLOT_0 &&
           item < MSG_SLOT_0 + zen::QuickReplies::CUSTOM_COUNT;
  }
  ...
  void renderItem(ZenDisplayDriver& display, int item, int y, bool sel) {
    ZenPrefs* p = _task->getNodePrefs();
    ...
    if (item == NOTIFICATION_MODE) {
      ...
      { uint8_t v = p ? p->notif_melody_dm : 0;
        display.print(zen::BuiltinMelodies::label(v)); }
      ...
      display.setCursor(valCol(display), y);
      { uint8_t v = p ? p->notif_melody_new_contact : zen::BuiltinMelodies::NONE;
        display.print(zen::BuiltinMelodies::label(v)); }
      ...
      const char* value = state == zen::FloodScopeView::APP_OVERRIDE ? "App" :
                          state == zen::FloodScopeView::APP_UNSCOPED ? "App Off" :
                          state == zen::FloodScopeView::DEFAULT ? "Default" : "None";
      display.setCursor(valCol(display), y);
      display.print(value);
      ...
      display.setCursor(valCol(display), y);
      if (p && p->timezone_mode == zen::TimezonePolicy::CITY) {
        display.print(zen::TimezonePolicy::cityName(p->timezone_city));
      } else {
        char buf[8];
      ...
      uint16_t bit = ZenPrefs::HP_FAVOURITES;
  ...
  void commitStagedChanges() {
    if (_scope_dirty) {
      if (the_mesh.setDefaultFloodScopeName(_scope_pending_name)) {
        ZenPrefs* saved = _task->getNodePrefs();
        if (saved) {
    ...
    if (gps_changed) {
      ...
      p->gps_interval = zen::GpsMode::pollingInterval(_gps_pending_polling);
      p->gps_adaptive = zen::GpsMode::pollingIsAdaptive(_gps_pending_polling);
    }
#endif
    ...
    // savePrefsIfDirty leaves its argument set when persistence fails. Retain
    // that pending state so shutdown or a later exit can retry the commit.
    _dirty = save_dirty;
    if (!save_dirty && p) {
...
public:
  ...
  void onShow() override {
    ZenPrefs* p = _task->getNodePrefs();
    // A failed previous commit keeps the original fingerprint so returning to
    // Settings and leaving again retries instead of accepting unsaved RAM as
    // the new baseline.
    if (!_dirty) {
    ...
    _gps_initial_polling = zen::GpsMode::pollingFromPrefs(
  ...
  int render(ZenDisplayDriver& display) override {
    ...
    if (_timezone_editor.active()) {
      ...
      return zenui::refreshMs(zenui::Refresh::Status);
    }
    ...
    return zenui::refreshMs(zenui::Refresh::Content);
  }
  ...
  bool handleInput(char c) override {
      ...
      if (r == DigitEditor::DONE) {
        ...
        if (!_child_pin_confirming) {
          ...
          _task->operationWarning(zen::Operation::CHILD_PIN,
              zen::OperationReason::PIN_MISMATCH);
        }
    ...
    if (_scope_editing) {
      ...
      if (result == KeyboardWidget::DONE) {
        _scope_editing = false;
        if (!zen::FloodScopeView::normaliseName(_kb->buf, _scope_proposed_name,
                                                  sizeof(_scope_proposed_name))) {
          _task->operationWarning(zen::Operation::FLOOD_SCOPE,
      ...
      if (res == PopupMenu::SELECTED && p) {
        switch (_picker.onSelected(_picker.menu.selectedIndex(), p, radioTarget(p))) {
          case RadioPresetPicker::START_SAVE:
            _kb->beginProfile(zen::EditorProfile::LITERAL, "",
                              (int)sizeof(p->user_radio_presets[0].name) - 1);
            break;
    ...
    if (zenui::moveWrapped(c, item_count, _section_sel)) return true;
    ...
    if (_selected == DM_MELODY && p && (left || right || enter)) {
      if (enter) _task->previewMelody(p->notif_melody_dm, zen::BuiltinMelodies::MESSAGE);
      else {
        p->notif_melody_dm = (p->notif_melody_dm + (left ? zen::BuiltinMelodies::COUNT - 1 : 1)) % zen::BuiltinMelodies::COUNT;
        _dirty = true;
      }
      return true;
    }
    if (_selected == CH_MELODY && p && (left || right || enter)) {
      if (enter) _task->previewMelody(p->notif_melody_ch, zen::BuiltinMelodies::KERPLOP);
      else {
        p->notif_melody_ch = (p->notif_melody_ch + (left ? zen::BuiltinMelodies::COUNT - 1 : 1)) % zen::BuiltinMelodies::COUNT;
        _dirty = true;
      }
      return true;
    }
    if (_selected == NEW_CONTACT_MELODY && p && (left || right || enter)) {
      if (enter) _task->previewMelody(p->notif_melody_new_contact, zen::BuiltinMelodies::NONE);
      else {
        p->notif_melody_new_contact = (p->notif_melody_new_contact +
            (left ? zen::BuiltinMelodies::COUNT - 1 : 1)) % zen::BuiltinMelodies::COUNT;
        _dirty = true;
      }
      return true;
    }
    if (_selected == AD_SOUND && p && (left || right || enter)) {
      if (enter) _task->previewMelody(p->notif_melody_ad, zen::BuiltinMelodies::MESSAGE);
      else {
        p->notif_melody_ad = (p->notif_melody_ad + (left ? zen::BuiltinMelodies::COUNT - 1 : 1)) % zen::BuiltinMelodies::COUNT;
        _dirty = true;
      }
      return true;
    }
    ...
    if (isHomePage(_selected) && p) {
      ...
      if (enter && homePageToggleable(_selected)) {
        if (!p->home_pages_mask) p->home_pages_mask = ZenPrefs::HP_ALL;
    ...
    if (_selected == DEFAULT_FLOOD_SCOPE && enter) {
      _kb->beginProfile(zen::EditorProfile::LITERAL, _scope_pending_name, 30);
      _scope_editing = true;
      return true;
    }
    ...
    if (_selected == CUSTOM_FREQ && p && enter) {
      _editor.beginFreq(p->freq, zen::RadioUiPolicy::MIN_FREQUENCY_MHZ,
                        zen::RadioUiPolicy::MAX_FREQUENCY_MHZ);
      return true;
    }
    ...
    if (_selected == GPS_POLLING && p && (left || right)) {
      ...
      if (left) choice = (choice + zen::GpsMode::POLLING_COUNT - 1) % zen::GpsMode::POLLING_COUNT;
      else choice = (choice + 1) % zen::GpsMode::POLLING_COUNT;
    ...
    if (_selected == DEVICE_NAME && p && enter) {
      _edit_name = true;
      _kb->beginProfile(zen::EditorProfile::LITERAL, the_mesh.getNodeName(),
                        (int)sizeof(p->node_name) - 1);
      return true;
    }
    ...
    if (_selected == CHILD_ROOMS && p && (left || right || enter)) {
      ...
      if (p->child_rooms_enabled)
        _task->operationWarning(zen::Operation::CHILD_MODE,
            zen::OperationReason::ROOMS_UNRESTRICTED);
      return true;
    }
#endif
    if (isMsgSlot(_selected) && enter) {
      ...
      messageeditor::beginQuickReply(
          *_kb, p ? p->custom_msgs[slot] : "",
          p ? (int)sizeof(p->custom_msgs[slot]) - 1 : KB_MAX_LEN, &sensors);
      return true;
    }
    return false;
  }
