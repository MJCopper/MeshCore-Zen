...
#include "../zen/QuickReplies.h"
#include "../zen/MessageDraftStore.h"
#include "../zen/MessageComposeSession.h"
#include "../zen/MessageSendState.h"
#include "../zen/MessageSendCoordinator.h"

class MessagesScreen : public ZenUIScreen {
  UITask* _task;
  ...
  zen::MessageSendState _send_state;
  ...
  uint8_t _active_replies[zen::QuickReplies::MAX_VISIBLE_COUNT];
  ...
  char      _pin_slot_labels[ZenPrefs::FAVOURITES_COUNT][40];  // "Slot N: " + full UTF-8 contact name
  ...
  // per-channel unread counters live in this store (see ConversationStore.h). The
  ...
  ConversationStore _history;
  PathDetailsView _path_view;
  zen::MessageDraftStore _drafts;
  zen::MessageComposeSession _compose;
  ...
  void setupMsgPick() {
    ...
    if (p) {
      for (uint8_t i = 0; i < zen::QuickReplies::CUSTOM_COUNT; i++) {
        if (p->custom_msgs[i][0] != '\0')
          _active_replies[_active_msg_count++] = zen::QuickReplies::BUILTIN_COUNT + i;
      }
    }
  }

  const char* quickReplyText(uint8_t entry) const {
    if (entry < zen::QuickReplies::BUILTIN_COUNT)
      return zen::QuickReplies::builtin(entry);
    ZenPrefs* p = _task->getNodePrefs();
    uint8_t slot = entry - zen::QuickReplies::BUILTIN_COUNT;
    return p && slot < zen::QuickReplies::CUSTOM_COUNT ? p->custom_msgs[slot] : "";
  }
  ...
  int sendTextLimit() const {
    return (int)zen::MessageTextPolicy::limit(MAX_TEXT_LEN,
        _sending_to_channel ? the_mesh.getNodeName() : nullptr);
  }

  void beginCustomMessage(bool replying = false) {
    char initial[zen::MessageDraftStore::TEXT_CAPACITY] = "";
    uint8_t reply_prefix_len = 0;
    bool restored = false;
    if (_sending_to_channel) _compose.selectChannel((uint8_t)_sel_channel_idx);
    else _compose.selectContact(_sel_contact.id.pub_key);
    if (!replying)
      restored = _compose.restore(initial, sizeof(initial), &reply_prefix_len);
    ...
    if (channel) {
      ...
      if (_retry_hist_pos >= 0 && the_mesh.getChannel(_sel_channel_idx, ch) &&
        ...
        if (e.ch_idx == (uint8_t)_sel_channel_idx && e.relay_status == ACK_FAIL) {
          ...
          ok = zen::MessageSendCoordinator::sendChannel(ch, body);
      ...
      if (current && zen::Policy::contactAllowed(_task->getNodePrefs(),
                                                  _task->isChildModeLocked(), current,
                                                  expected))
        ok = _history.resendFailedDM(_retry_hist_pos, _sel_contact.id.pub_key);
    }
    _retry_hist_pos = -1;
    if (!ok) _task->operationError(zen::Operation::MESSAGE,
        zen::OperationOutcome::TRANSPORT_FAILURE, zen::OperationReason::NOT_QUEUED);
  ...
  void afterSend(bool ok, const char* msg) {
    ...
    if (ok && _sending_to_channel) {
      ...
      _history.storeDMMsg(_sel_contact.id.pub_key, true, msg, _send_state.ack_tag,
                          _send_state.ack_deadline_ms, _send_state.timestamp,
                          _send_state.route);
      ...
      if (_sending_to_channel) {
        ...
        snprintf(entry, sizeof(entry), "Me: %s", msg);
        int pos = addChannelMsg(_sel_channel_idx, entry);
        _history.failChannelRelay(pos);
        _history.setChUnread(_sel_channel_idx, 0);
        _unread_at_entry = 0;
        _viewing_max_seen = 0;
      } else {
        _history.storeFailedDM(_sel_contact.id.pub_key, msg, _send_state.timestamp,
                               _send_state.route);
        _task->reconcileDMUnread();
      ...
      _task->operationError(zen::Operation::MESSAGE,
          zen::OperationOutcome::TRANSPORT_FAILURE, zen::OperationReason::NOT_QUEUED);
    }
  }
  ...
  bool sendText(const char* msg) {
    ...
    if (_sending_to_channel) {
      ...
      return zen::MessageSendCoordinator::sendChannel(ch, msg);
    } else {
      ...
      return zen::MessageSendCoordinator::sendDirect(_sel_contact, msg,
                                                       _send_state);
    }
  }
  ...
  void buildContactList() {
    ...
    for (int i = 0; i < total; i++) {
        ...
        if (_task->isChildModeLocked() &&
            !zen::Policy::contactAllowed(p, true, &c,
  ...
  bool channelAllowedForChild(uint8_t index, const ChannelDetails& ch) const {
    return zen::Policy::channelAllowed(_task->getNodePrefs(), _task->isChildModeLocked(),
  ...
  bool toggleContactFavourite(const ContactInfo& contact) {
    ...
    int slot = _task->findFavouriteSlot(contact.id.pub_key);
    if (favourite && slot < 0) {
      for (int candidate = 0; candidate < ZenPrefs::FAVOURITES_DIAL_COUNT; candidate++) {
        if (_task->isFavouriteSlotEmpty(candidate)) {
          slot = candidate;
          break;
        }
      }
      if (slot < 0) {
        _task->operationWarning(zen::Operation::FAVOURITES,
                                zen::OperationReason::LIST_FULL);
        return false;
      }
    }

    ZenPrefs* prefs = _task->getNodePrefs();
    uint8_t previous[ZenPrefs::FAVOURITE_PREFIX_LEN] = {};
    if (prefs && slot >= 0)
      memcpy(previous, prefs->favourite_contacts[slot], sizeof(previous));
    if (!the_mesh.setContactFavourite(contact.id.pub_key, favourite)) return false;
    if (favourite && !_task->setFavouriteSlot(slot, contact.id.pub_key)) {
      the_mesh.setContactFavourite(contact.id.pub_key, false);
      return false;
    }
    if (!favourite && slot >= 0) _task->clearFavouriteSlot(slot);
    if (!the_mesh.savePrefs()) {
      if (prefs && slot >= 0)
        memcpy(prefs->favourite_contacts[slot], previous, sizeof(previous));
      the_mesh.setContactFavourite(contact.id.pub_key, !favourite);
      return false;
    }
  ...
  uint8_t chNotifState(uint8_t ch_idx) const {
    ZenPrefs* p = _task->getNodePrefs();
    return zen::NotificationPreferences::channelState(p, ch_idx);
  }
  bool setChNotifState(uint8_t ch_idx, uint8_t state) {
    ZenPrefs* p = _task->getNodePrefs();
    return zen::NotificationPreferences::setChannelState(p, ch_idx, state);
  }

  uint8_t dmNotifState(const uint8_t* pub_key) const {
    ZenPrefs* p = _task->getNodePrefs();
    return zen::NotificationPreferences::dmState(p, pub_key);
  }
  bool setDmNotifState(const uint8_t* pub_key, uint8_t state) {
    ZenPrefs* p = _task->getNodePrefs();
    return zen::NotificationPreferences::setDmState(p, pub_key, state);
  }
  ...
  static const char* melodyOverrideLabel(uint8_t value) {
    return value ? zen::BuiltinMelodies::label(value - 1) : "Global";
  }

  uint8_t chNotifMelody(uint8_t ch_idx) const {
    ZenPrefs* p = _task->getNodePrefs();
    return zen::NotificationPreferences::channelMelody(p, ch_idx);
  }
  bool setChNotifMelody(uint8_t ch_idx, uint8_t slot) {
    ZenPrefs* p = _task->getNodePrefs();
    return zen::NotificationPreferences::setChannelMelody(p, ch_idx, slot);
  }

  uint8_t dmMelodySlot(const uint8_t* pub_key) const {
    ZenPrefs* p = _task->getNodePrefs();
    return zen::NotificationPreferences::dmMelody(p, pub_key);
  }
  bool setDmMelody(const uint8_t* pub_key, uint8_t slot) {
    ZenPrefs* p = _task->getNodePrefs();
    return zen::NotificationPreferences::setDmMelody(p, pub_key, slot);
  }
...
public:
  ...
  MessagesScreen(UITask* task, KeyboardWidget* kb)
    : _task(task), _kb(kb), _phase(MODE_SELECT), _mode_sel(0),
      ...
      _ch_view(task) {
    // The history rings + per-channel unread counters init in ConversationStore.
  }

  // A blank name is still an occupied slot when it has a channel key.
  int findFreeChannelSlot() const {
    return zen::ChannelSlotPolicy::firstFree<ChannelDetails>(the_mesh, MAX_GROUP_CHANNELS);
  }

  // CHANNEL_PICK row count including the synthetic "+ Add channel" row.
  int channelPickTotal() const {
    return _num_channels + (_task->isChildModeLocked() ? 0 : 1);
  }
  ...
  bool startNodeLogin(const char* password, bool used_saved_password = false) {
    bool sent = _task->startNodeLogin(zen::NodeLoginCoordinator::MESSAGES,
                                      _sel_contact, password, used_saved_password);
    if (!sent)
      _task->operationError(zen::Operation::ROOM_LOGIN,
  ...
  void onNodeLoginResult(const uint8_t* pub_key, bool success, uint8_t permissions) {
    (void)permissions;
    if (_phase != ROOM_LOGIN_WAIT ||
        memcmp(_sel_contact.id.pub_key, pub_key, PUB_KEY_SIZE) != 0) return;
    if (success) {
      openDmHistory();
    } else {
      _task->operationError(zen::Operation::ROOM_LOGIN,
      ...
      _login_mode = true;
      _kb->beginProfile(zen::EditorProfile::CREDENTIAL, "", 15);
      _phase = KEYBOARD;
    }
  ...
  void onNodeLoginTimeout(const uint8_t* pub_key) {
    if (_phase != ROOM_LOGIN_WAIT ||
        memcmp(_sel_contact.id.pub_key, pub_key, PUB_KEY_SIZE) != 0) return;
    _task->operationError(zen::Operation::ROOM_LOGIN,
        zen::OperationOutcome::TIMEOUT, zen::OperationReason::NO_REPLY);
    _login_mode = true;
    _kb->beginProfile(zen::EditorProfile::CREDENTIAL, "", 15);
    _phase = KEYBOARD;
  }
  ...
  // clamp the unread coordinator's counters to what the shared 32-slot DM
  ...
  int render(ZenDisplayDriver& display) override {
    ...
    if (_phase == MODE_SELECT) {
      ...
      if (!_task->remoteStatus(zen::RemoteNodeCoordinator::LOGIN_OWNER,
                               status, sizeof(status))) strcpy(status, "Logging in...");
      display.drawTextCentered(display.width() / 2, start_y + item_h * 2, status);

    } else if (_phase == CHANNEL_PICK) {
      display.drawCenteredHeader("Select Channel", true, _ctx_menu.active);

      // "+ Add channel" is a synthetic trailing row.
      bool show_add = !_task->isChildModeLocked();
  ...
  bool handleInput(char c) override {
    ...
    if (_phase == MODE_SELECT) {
        ...
        if (!_pin_picker_active && _num_contacts > 0) {
          ...
          if (c == KEY_ENTER && value_sel == 2) {
            ...
            if (p && the_mesh.getContactByIdx(_sorted[_contact_sel], ci)) {
              uint8_t selection = zen::BuiltinMelodies::resolveOverride(
                  dmMelodySlot(ci.id.pub_key), p->notif_melody_dm);
              _task->previewMelody(selection, zen::BuiltinMelodies::MESSAGE);
            }
            return true;
          }
            ...
            if (the_mesh.getContactByIdx(_sorted[_contact_sel], ci)) {
              ...
              if (sel == 1) {
                ...
                if (!setDmNotifState(ci.id.pub_key, v)) {
                  _task->operationWarning(zen::Operation::RINGTONE,
                      zen::OperationReason::LIST_FULL);
                  return true;
                }
                ...
                v = right ? (v + 1) % (zen::BuiltinMelodies::COUNT + 1)
                          : (v + zen::BuiltinMelodies::COUNT) % (zen::BuiltinMelodies::COUNT + 1);
          ...
          if (the_mesh.getContactByIdx(_sorted[_contact_sel], ci)) {
            ...
            if (sel == 0) {
              ...
              if (pinned_slot >= 0) {
                ZenPrefs* prefs = _task->getNodePrefs();
                uint8_t previous[ZenPrefs::FAVOURITE_PREFIX_LEN];
                memcpy(previous, prefs->favourite_contacts[pinned_slot], sizeof(previous));
                _task->clearFavouriteSlot(pinned_slot);
                if (!the_mesh.saveZenPrefs()) {
                  memcpy(prefs->favourite_contacts[pinned_slot], previous, sizeof(previous));
                  return true;
                }
                char alert[24];
                ...
                for (int s = 0; s < ZenPrefs::FAVOURITES_DIAL_COUNT; s++) {
                ...
                for (int s = 0; s < ZenPrefs::FAVOURITES_DIAL_COUNT; s++) _ctx_menu.addItem(_pin_slot_labels[s]);
        ...
        if (!_channel_delete_confirm_active && _num_channels > 0) {
          ...
          if (c == KEY_ENTER && value_sel == 2) {
            ...
            if (p) {
              ...
              _task->previewMelody(selection, zen::BuiltinMelodies::KERPLOP);
            }
            return true;
          }
          ...
          if (left || right) {
            ...
            if (sel == 1) {
              ...
              ZenPrefs* p2 = _task->getNodePrefs();
              if (p2) {
        ...
        if (_channel_delete_confirm_active) {
          if (res == PopupMenu::SELECTED && _ctx_menu.selectedIndex() == 0) {
            ...
            if (the_mesh.setChannelLocal(_ctx_ch_idx, ch) == MyMesh::CHANNEL_SAVED)
              ...
              _task->operationError(zen::Operation::CHANNEL,
                  zen::OperationOutcome::TRANSPORT_FAILURE,
      ...
      if (c == KEY_ENTER && _channel_sel == _num_channels) {
        if (_task->isChildModeLocked()) return true;
        int idx = findFreeChannelSlot();
        if (idx < 0) _task->operationWarning(zen::Operation::CHANNEL,
            zen::OperationReason::LIST_FULL);
      ...
      if (c == KEY_CANCEL) {
        _task->cancelNodeLogin(zen::NodeLoginCoordinator::MESSAGES, _sel_contact.id.pub_key);
        _phase = CONTACT_PICK;
      }
      return true;
    } else if (_phase == KEYBOARD) {
      ...
      if (res == KeyboardWidget::CANCELLED) {
        ...
        if (_kb->len > prefix_len) {
          ...
          zen::MessageTextPolicy::trim(expanded, sendTextLimit());
          clearCurrentDraft();
      ...
      if (c == KEY_ENTER) {
        ...
        zen::MessageTextPolicy::trim(msg, sendTextLimit());
