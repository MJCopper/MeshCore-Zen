...
class RingtoneEditorScreen : public ZenUIScreen {
  UITask* _task;
  ZenPrefs* _prefs;
  static const int MAX_NOTES = zen::RingtoneModel::MAX_NOTES;
  ...
  void previewNote(uint8_t note) {
    if (!zen::RingtoneModel::pitchIndex(note)) { _task->stopMelody(); return; }
    zen::RingtoneModel::buildRTTTL(&note, 1, 4, _play_buf, sizeof(_play_buf));
  ...
  bool saveCurrent() {
    ...
    memset(stored, 0, zen::RingtoneModel::STORAGE_NOTES);
...
public:
  RingtoneEditorScreen(UITask* task, ZenPrefs* prefs)
  ...
  void selectSlot(int slot = 0) {
    ...
    _bpm_idx = bpm < zen::RingtoneModel::BPM_COUNT ? bpm : 2;
  ...
  void openMenu() {
    ...
    uint8_t duration = _cursor < _len ? zen::RingtoneModel::duration(_notes[_cursor]) : 0;
    snprintf(_menu_dur_label, sizeof(_menu_dur_label), "Duration: %s", zen::RingtoneModel::durationLabel(duration));
    snprintf(_menu_bpm_label, sizeof(_menu_bpm_label), "BPM: %u", zen::RingtoneModel::bpm(_bpm_idx));
  ...
  int render(ZenDisplayDriver& display) override {
    ...
    snprintf(header, sizeof(header), "Custom%d%s %u %d/%d", _slot + 1, dirty() ? "*" : "",
             zen::RingtoneModel::bpm(_bpm_idx), _len, MAX_NOTES);
    ...
    for (int i = 0; i < _visible_notes; i++) {
      ...
      if (ni < _len) {
        char label[5];
        zen::RingtoneModel::label(_notes[ni], label, sizeof(label));
    ...
    if (_cursor < _len) {
      char info[24];
      snprintf(info, sizeof(info), "oct:%u dur:%s", zen::RingtoneModel::octave(_notes[_cursor]),
               zen::RingtoneModel::durationLabel(zen::RingtoneModel::duration(_notes[_cursor])));
  ...
  bool handleInput(char c) override {
    ...
    if (_menu.active) {
      ...
      if (left || right) {
        if (selected == MI_DURATION && _cursor < _len) {
          uint8_t value = zen::RingtoneModel::duration(_notes[_cursor]);
          value = right ? (value + 1) % zen::RingtoneModel::DURATION_COUNT
                        : (value + zen::RingtoneModel::DURATION_COUNT - 1) % zen::RingtoneModel::DURATION_COUNT;
          _notes[_cursor] = zen::RingtoneModel::withDuration(_notes[_cursor], value);
          snprintf(_menu_dur_label, sizeof(_menu_dur_label), "Duration: %s", zen::RingtoneModel::durationLabel(value));
        } else if (selected == MI_BPM) {
          if (right && _bpm_idx + 1 < zen::RingtoneModel::BPM_COUNT) _bpm_idx++;
      ...
      if (result == PopupMenu::SELECTED) {
        switch ((MenuIdx)_menu.selectedIndex()) {
          case MI_PLAY:
            ...
            else if (_len) { zen::RingtoneModel::buildRTTTL(_notes, _len, _bpm_idx, _play_buf, sizeof(_play_buf)); _task->playMelody(_play_buf); }
            break;
          ...
          case MI_INSERT:
            if (_len < MAX_NOTES) {
              ...
              _notes[at] = zen::RingtoneModel::pack(1, 5, 1);
    ...
    if ((up || down || enter) && _cursor == _len && _len < MAX_NOTES) {
      _notes[_len++] = zen::RingtoneModel::pack(1, 5, 1);
      clampScroll(); previewNote(_notes[_cursor]); return true;
    }
    if ((up || down) && _cursor < _len) {
      uint8_t pitch = zen::RingtoneModel::pitchIndex(_notes[_cursor]);
      pitch = up ? (pitch + 1) % zen::RingtoneModel::PITCH_COUNT
                 : (pitch + zen::RingtoneModel::PITCH_COUNT - 1) % zen::RingtoneModel::PITCH_COUNT;
      _notes[_cursor] = zen::RingtoneModel::withPitch(_notes[_cursor], pitch);
      previewNote(_notes[_cursor]); return true;
    }
    if (enter && _cursor < _len) {
      uint8_t octave = zen::RingtoneModel::octave(_notes[_cursor]);
      if (zen::RingtoneModel::pitchIndex(_notes[_cursor])) octave = octave < 7 ? octave + 1 : 4;
      _notes[_cursor] = zen::RingtoneModel::withOctave(_notes[_cursor], octave);
      previewNote(_notes[_cursor]); return true;
    }
    return false;
  }
