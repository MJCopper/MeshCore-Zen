...
struct FullscreenMsgView {
  ...
  // ZenDisplayDriver::getCodepointWidth — O(n) instead of O(n²) substring
  // re-measurement. Works for both fixed- and variable-width fonts.
  static int wrapLines(ZenDisplayDriver& display, const char* text, int max_px,
    ...
    while (*seg && count < max_lines) {
      ...
      while (*p && (p - seg) < FS_CHARS_MAX - 1) {
        ...
        uint32_t cp = ZenDisplayDriver::decodeCodepoint(p);
  ...
  int render(ZenDisplayDriver& display, const char* sender, const char* text,
    ...
    int lcount = wrapLines(display, body, max_px, s_wrap_lines, MSG_WRAP_LINES_MAX);
    ...
    if (reserve > 0)
      lcount = wrapLines(display, body, max_px - reserve, s_wrap_lines, MSG_WRAP_LINES_MAX);
