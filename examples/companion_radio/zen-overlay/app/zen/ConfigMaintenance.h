...
class ConfigMaintenance {
  ...
  static bool compactTable(Entry* table, int count, uint8_t Entry::* value,
                           uint8_t maximum) {
    Entry clean[ZenPrefs::DM_NOTIF_TABLE_MAX];
  ...
  static bool applySchemaMigrations(ZenPrefs& prefs) {
    bool changed = false;
    while (prefs.zen_config_schema < CURRENT_SCHEMA) {
      switch (prefs.zen_config_schema) {
        ...
        case 1:
          ...
          // Clock Tools or user-GPIO features. Keep their bytes in ZenPrefs so
        ...
        case 3:
          ...
          break;
        case 4:
          // MeshCore owns GPS enablement and polling. Zen migrations must not
          // rewrite those baseline preferences.
...
public:
  ...
  static bool migrateMelodySchema(ZenPrefs& prefs, uint32_t file_schema) {
    ...
    for (int i = 0; i < ZenPrefs::DM_MELODY_TABLE_MAX; i++)
  ...
  static bool apply(ZenPrefs& prefs) {
    // Ordered semantic migrations run before current-schema validation. This
    // prevents a current validator from destroying a value an older schema
    // still needs in order to migrate it correctly.
    bool changed = applySchemaMigrations(prefs);
    if (prefs.display_brightness > 4) { prefs.display_brightness = 4; changed = true; }
    if (prefs.auto_off_secs != 0 && prefs.auto_off_secs != 5 && prefs.auto_off_secs != 15 &&
        prefs.auto_off_secs != 30 && prefs.auto_off_secs != 60) {
      prefs.auto_off_secs = 15;
      changed = true;
    }
    if (prefs.batt_display_mode > 2) { prefs.batt_display_mode = 0; changed = true; }
    if (prefs.clock_hide_seconds > 1) { prefs.clock_hide_seconds = 0; changed = true; }
    if (prefs.clock_12h > 1) { prefs.clock_12h = 1; changed = true; }
    if (prefs.use_lemon_font > 1) { prefs.use_lemon_font = 0; changed = true; }
    if (prefs.units_imperial > 1) { prefs.units_imperial = 0; changed = true; }
    if (prefs.display_rotation > 3) { prefs.display_rotation = 0; changed = true; }
    if (prefs.joystick_rotation > 3) { prefs.joystick_rotation = 0; changed = true; }
    if (prefs.eink_full_refresh_every > 4) { prefs.eink_full_refresh_every = 0; changed = true; }
    uint16_t valid_pages = prefs.home_pages_mask & ZenPrefs::HP_ALL;
    if (prefs.home_pages_mask != valid_pages) { prefs.home_pages_mask = valid_pages; changed = true; }
    if (prefs.page_order_set != 0 && prefs.page_order_set != ZenPrefs::PAGE_ORDER_MAGIC) {
      prefs.page_order_set = 0;
      changed = true;
    }
    for (uint8_t i = 0; i < ZenPrefs::PAGE_ORDER_LEN; i++) {
      if (prefs.page_order[i] > ZenPrefs::HPB_COUNT) { prefs.page_order[i] = 0; changed = true; }
    }
    if (prefs.buzzer_auto > 1) { prefs.buzzer_auto = 0; changed = true; }
    if (prefs.buzzer_volume > 4) { prefs.buzzer_volume = 4; changed = true; }
    if (prefs.advert_sound_scope > 1) { prefs.advert_sound_scope = 0; changed = true; }
    if (prefs.dm_show_all > 1) { prefs.dm_show_all = 1; changed = true; }
    if (prefs.room_fav_only > 1) { prefs.room_fav_only = 0; changed = true; }
    if (prefs.ch_fav_only > 1) { prefs.ch_fav_only = 0; changed = true; }
    for (uint8_t i = 0; i < 5; i++) {
      if (prefs.custom_msgs[i][sizeof(prefs.custom_msgs[i]) - 1] != '\0') {
        prefs.custom_msgs[i][sizeof(prefs.custom_msgs[i]) - 1] = '\0';
        changed = true;
      }
    }
    for (uint8_t i = 0; i < ZenPrefs::USER_RADIO_PRESET_MAX; i++) {
      ZenPrefs::UserRadioPreset& preset = prefs.user_radio_presets[i];
      if (preset.name[sizeof(preset.name) - 1] != '\0') {
        preset.name[sizeof(preset.name) - 1] = '\0';
        changed = true;
      }
      if (preset.name[0] && (!isfinite(preset.freq) || preset.freq < 150 || preset.freq > 2500 ||
                             !isfinite(preset.bw) || preset.bw < 7.8f || preset.bw > 500 ||
                             preset.sf < 5 || preset.sf > 12 || preset.cr < 5 || preset.cr > 8)) {
        memset(&preset, 0, sizeof(preset));
        changed = true;
      }
    }
    if (prefs.advert_auto_interval_sec != 0 && prefs.advert_auto_interval_sec != 3600 &&
        prefs.advert_auto_interval_sec != 10800 && prefs.advert_auto_interval_sec != 21600) {
      prefs.advert_auto_interval_sec = 0;
      changed = true;
    }
    ...
    changed |= compactTable(prefs.dm_notif, ZenPrefs::DM_NOTIF_TABLE_MAX,
                            &ZenPrefs::DmNotifEntry::state, 2);
    changed |= compactTable(prefs.dm_melody, ZenPrefs::DM_MELODY_TABLE_MAX,
                            &ZenPrefs::DmMelodyEntry::slot, BuiltinMelodies::COUNT);
    for (uint8_t i = 0; i < 64; i++) {
