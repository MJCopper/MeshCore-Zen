...
struct ZenPrefs : public NodePrefs {  // Zen-only extension persisted separately
  ...
  // slots 5..9 are reserved legacy memory and are not persisted by ZenStore.
  ...
  // Global selection IDs from zen/BuiltinMelodies.h.
  ...
  // Declared as a literal (not HPB_COUNT) so adding a page does not silently
  // change the current Zen record shape.
  ...
  // On-device delivery uses the fixed policy in zen/NodeRouteRetry.h.
  ...
  // Retired command bytes retained only as unused in-memory compatibility data.
  ...
  // no ADC channel). These retired fields are not persisted by ZenStore.
  uint8_t  gpio1_mode;
  uint8_t  gpio2_mode;
  uint8_t  gpio3_mode;
  uint8_t  gpio4_mode;
  ...
  uint8_t bluetooth_enabled;  // persisted BLE companion state
  ...
  // Semantic schema stored in the ZenPrefsCodec envelope. MeshCore preference
  // schema and files are independent of this value.
  ...
  ZenPrefs() : NodePrefs() {
    // Do not clear the NodePrefs base: it owns ConfigSerializer children whose
    // parent pointers are established by its constructor. Only the derived
    // Zen storage region is plain preference data.
    uint8_t* first = &buzzer_volume;
    memset(first, 0, sizeof(ZenPrefs) - (size_t)(first - (uint8_t*)this));
  }

};

// ZenPrefs is never written as a raw object. ZenPrefsCodec explicitly selects
// Zen-owned fields, so MeshCore's inherited serializer and object layout stay
// outside the Zen record.
static_assert(sizeof(ZenPrefs) < 4096,
              "ZenPrefs unexpectedly exceeds the bounded external-flash record size.");
