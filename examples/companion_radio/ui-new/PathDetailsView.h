...
class PathDetailsView {
  ...
  int _scroll = 0;
  solo::PathAttemptSnapshot _attempt{};
...
public:
  ...
  void setAttempt(const solo::PathAttemptSnapshot& attempt) { _attempt = attempt; }

  void open(const uint8_t* key, const solo::PathAttemptSnapshot& attempt = {}) {
  ...
  int render(DisplayDriver& display) {
    ...
    solo::PathShape shape = solo::pathShape(target.out_path_len, sizeof(target.out_path));
